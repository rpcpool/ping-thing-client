// This script sends self sol transfer transactions to itself
// The env vars needed are
/*

WALLET_PRIVATE_KEYPAIR
VA_API_KEY - olny if you want to send the results to ping thing
CUSTOM_SEND_TX_ENDPOINT
RPC_ENDPOINT
WS_ENDPOINT
...
const SHIELD_POLICIES = JSON.parse(process.env.SHIELD_POLICIES!) || [""];
let shieldPolicyIndex = 0;
...
const resultFilePath = `./results/${Date.now()}`;

let leaderSchedule = null;
let epochInfo = null;
...
async function pingThing() {
  ...
  console.log(`Tx send endpoint Endpoint: ${CUSTOM_SEND_TX_ENDPOINT}`);

  leaderSchedule = await getLeaderScheduleWithAddedOffset();
  epochInfo = await getEpochInfo();

  // await appendToCsv(`./results/leaderSchedule-${Date.now()}`, JSON.stringify(leaderSchedule))
  ...
  while (true) {
    ...
    let shieldPolicies = [SHIELD_POLICIES[shieldPolicyIndex]];
    ...
    try {
      try {
        ...
        const finalTxToSendToRpcAfterAllTheFuckloadOfEncodings = bs58.encode(
          getTransactionEncoder().encode(
            transactionSignedWithFeePayer
          ) as Uint8Array
        );

        const sendTxResult = await sendTx(
          finalTxToSendToRpcAfterAllTheFuckloadOfEncodings,
          shieldPolicies
        );
        ...
        while (true) {
          try {
            ...
            const sigStatus = await rpcConnection
              .getSignatureStatuses([signature])
              .send();
            if (!sigStatus.value[0]?.slot) {
              await sleep(2000);
              throw new Error("TxSendTimeout");
            }
            break;
            ...
            if (diffSeconds >= 10) {
              console.log("10 seconds have passed, aborting the tx sending");
              throw e;
            }
        ...
        appendToCsv(
          resultFilePath,
          `\n\n${SHIELD_POLICIES[shieldPolicyIndex]},-1,null,${signature}`
        );
        incrementShieldIndex();
      ...
      if (signature !== FAKE_SIGNATURE) {
        ...
        const slotLeader = await getLeaderForSlot(
          leaderSchedule,
          Number(txLanded.slot),
          epochInfo!
        );
        console.log("SLOT LEADER");
        console.log(slotLeader);

        appendToCsv(
          resultFilePath,
          `\n\n${SHIELD_POLICIES[shieldPolicyIndex]},${txLanded.slot},${slotLeader},${signature}`
        );
        incrementShieldIndex();
...
async function sendTx(tx: string, shieldPolicies: string[]) {
  const endpoint =
    CUSTOM_SEND_TX_ENDPOINT != null ? CUSTOM_SEND_TX_ENDPOINT : RPC_ENDPOINT!;

  console.log("SEND TX ENDPOINT");
  ...
  const txBody = JSON.stringify({
    ...
    params: [
      tx,
      {
        skipPreflight: true,
        maxRetries: 0,
        forwardingPolicies: shieldPolicies,
      },
    ],
  });
  ...
  console.log("SEND TX RESPONSE HEADERS");
  console.log(res.headers);

  console.log("SEND TX RESPONE JSON");
  console.log(json);

  return json.result;
}

async function getLeaderForSlot(
  leaderSchedule: any,
  slot: number,
  epochInfo: { epoch: number }
) {
  const epochOffset = epochInfo.epoch * 432000;
  for (const [key, slots] of Object.entries(leaderSchedule)) {
    if (Array.isArray(slots)) {
      const adjustedSlots = slots.map((s: number) => s + epochOffset);
      if (adjustedSlots.includes(slot)) {
        return key;
      }
    }
  }
...
async function getLeaderScheduleWithAddedOffset() {
...
function incrementShieldIndex() {
  shieldPolicyIndex++;
  if (shieldPolicyIndex >= SHIELD_POLICIES.length) {
    shieldPolicyIndex = 0;
  }
}
